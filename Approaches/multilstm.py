...
mdl = Sequential()
mdl.add(LSTM(units=FEED_LEN, return_sequences=True, input_shape=(FEED_LEN, 4)))
...
historyLstm = mdl.fit(X_train, Y_train, epochs=5, batch_size=64, validation_data=(x_test, y_test))

mdl.save('Models/lstm.h5')
...
plt.savefig('Results/multi_lstm_model_loss.jpg')
plt.show()
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig('Results/multi_lstm_mape.jpg')
plt.show()
...
plt.savefig('Results/multi_lstm_actual_vs_predicted.jpg')
plt.show()
